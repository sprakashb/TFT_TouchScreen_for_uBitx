...
#define TX_LPF_B 17
#define TX_LPF_C 18
#define CW_KEY 19     // 20,21 are SDA/SCL so all pins in sequence on Arduino Mega 
...
bool active_PTT_in = true; // if PTT remains continuously low during QSO make it true else
...
uint32_t bfo2 = 11996100L;  // Fixed 12 MHz BFO2 Farhans 11996500 this value found by test
uint32_t bfo1, bfo1_LSB = 56995000L , bfo1_USB = 32995000L;   // Initial Values of BFO1 for USB or LSB
...
uint8_t magic_no = 02; // used for checking the initialization of eprom or to reinitialize change this no.
...
int svx, svy, svwd, svht, f1x, f1y, f1wd, f1ht, f2x, f2y, f2wd, f2ht, f3x, f3y, f3wd, f3ht; //F2 made longer for bfo2
int f4x, f4y, f4wd, f4ht; // F4 no longer displayed
...
#ifdef REJI5408
identifier = 0x9320;    //0x5408; // Rejimon's 2.4 DISPLAY from Robodo detected as 5408 but works as 9320
// Regimons display
TS_LEFT = 74;
TS_RT  = 906;
TS_TOP = 117 ;
...
void loop()
{
  ...
  /* if (active_PTT_in)         // normal PTT goes to ground on press for short time
    ...
    #ifdef debug
     Serial.println(1);
     //Serial.println(digitalRead(PTT_Input));
    #endif
     rx_tx_ptt();
    }
  ...
  //TBT
  // for uBitx PTT
  if (active_PTT_in)     // i.e. Normal type PTT we call active PTT
  {
    if (digitalRead(PTT_Input) == LOW )
      ptt_ON();
    else
...
ypos = map(tp.x, TS_TOP, TS_BOT, 0, tft.height());
    ...
    if (ypos > firstrowy && ypos < firstrowy + buttonht)  // first row of buttons (orig 5,42)
    {
      ...
      else if (xpos > txrx  && xpos < txrx + txrwd - 2 ) // toggle between Rx & Tx, TX_RX (D14) goes Hi on Tx
      { toggle_ptt();
        delay(100);
      }
      return;
